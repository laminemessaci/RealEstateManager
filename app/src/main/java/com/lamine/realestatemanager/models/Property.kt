...
data class Property (
    ...
    fun fromContentValues(values: ContentValues): Property {
        ...
        if (values.containsKey("description")) property.description =
            values.getAsString("description")
        if (values.containsKey("price")) property.price = values.getAsDouble("price")
        if (values.containsKey("livingSpace")) property.livingSpace =
            values.getAsInteger("livingSpace")
        ...
        if (values.containsKey("trainStation")) property.trainStation =
            values.getAsBoolean("trainStation")
        ...
        if (values.containsKey("dateOfEntry")) property.dateOfEntry =
            values.getAsString("dateOfEntry")
        if (values.containsKey("dateOfSale")) if (values.getAsString("dateOfSale") == "") property.dateOfSale =
            null else property.dateOfSale = values.getAsString("dateOfSale")
        ...
        if (values.containsKey("apartmentNumber")) if (values.getAsInteger("apartmentNumber") == 0) property.address?.apartmentNumber =
            null else property.address?.apartmentNumber = values.getAsInteger("apartmentNumber")
        if (values.containsKey("city")) property.address?.city = values.getAsString("city")
        if (values.containsKey("postalCode")) property.address?.postalCode =
            values.getAsString("postalCode")
        if (values.containsKey("country")) property.address?.country = values.getAsString("country")
        if (values.containsKey("additionalAddress")) property.address?.additionalAddress =
            values.getAsString("additionalAddress")
        if (values.containsKey("lat")) if (values.getAsDouble("lat") == 0.0) property.address?.lat =
            null else property.address?.lat = values.getAsDouble("lat")
        if (values.containsKey("lng")) if (values.getAsDouble("lng") == 0.0) property.address?.lng =
            null else property.address?.lng = values.getAsDouble("lng")
        return property
    }
}
